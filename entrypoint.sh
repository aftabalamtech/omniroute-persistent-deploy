...
# A database file is not automatically a valid OmniRoute database. In particular,
# bootstrap-env.mjs requires provider_connections to exist. If a stale/incomplete
# database is present, keep it untouched and recover only from a fully validated
# remote backup.
database_needs_restore() {
  ...
  if ! sqlite3 "$DB_PATH" "SELECT 1 FROM sqlite_master WHERE type='table' AND name='provider_connections' LIMIT 1;" 2>/dev/null | grep -qx '1'; then
    printf '[restore] existing database is missing required table provider_connections; recovery required\n' >&2
    return 0
  fi
...
if [[ -f "$DB_PATH" ]]; then
  if database_needs_restore; then
    ...
    printf '[restore] existing database validated; skipping restore\n'
  fi
else
...
# This avoids taking SQLite snapshots while OmniRoute is still migrating.
